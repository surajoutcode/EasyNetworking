    ...
    public func encode(_ urlRequest: URLRequestConvertible, with parameters: Parameters?) throws -> URLRequest {
        ...
        for (key, value) in parameters {
            if let imageData = value as? Data {
                ...
                body.append("Content-Disposition: form-data; name=\"\(key)\"; filename=\"\(arc4random()).\(key == "doc" ? "docx" : key == "video" ? "mp4" : "jpg")\"\(lineBreak)")
                body.append("Content-Type: \(key == "doc" ? "application/octet-stream" : key == "video" ? "application/mp4" : "image/jpeg")\(lineBreak + lineBreak)")
